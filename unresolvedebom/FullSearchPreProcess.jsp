...
boolean boolReplaceOperValidation = true;
...
String selPartRowId 		= "";
String is3DSearch = "true";
try{
	is3DSearch = EnoviaResourceBundle.getProperty(context, "emxFramework.Enable3DSearch");
}
catch(Exception e){
	is3DSearch = "true";
}
...
//isWipBomAllowed = Boolean.toString(EngineeringConstants.DEVELOPMENT.equals(UnresolvedEBOM.isWipBomAllowedForParts(context,parentOID)));
...
<script language="Javascript">
	...
	var isFROMRMB = "<xss:encodeForJavaScript><%=isFromRMB%></xss:encodeForJavaScript>";
	//XSSOK
  ...
  //End ----- of DragNDrop
    ...
    //2012x--Ends
     ...
     try{
            ...
            if ((calledMethod == "createAndassignPUEECO" || calledMethod == "addToExistingPUEECO" || calledMethod == "createAndAssignChangeOrder") && isWipBomAllowed.toLowerCase()=="true")
            {
	            ...
	            if (rowsSelected.length > 0){
                    ...
                    for(var i=0; i<rowsSelected.length; i++) {
                          ...
                          //XSSOK
                            if ((objState != "<%=UnresolvedEBOMConstants.STATE_PART_PRELIMINARY%>") || columnAddVal != "" || columnPolicyVal != "Configured Part") {
		...
		if (calledMethod == "createAndAssignChangeOrder" && isWipBomAllowed.toLowerCase() != "true") {
			selectedParts = "fromConfiguredViewMode";
		}            
...
<%
    //2012x-Starts
     ...
     if("".equals(contextECO) && !"addToProduct".equals(calledMethod) && "false".equalsIgnoreCase(isWipBomAllowed) && !"createAndAssignChangeOrder".equals(calledMethod)) {
%>
		    <script language = "javascript">
			    if(warningMessage == ""){
			    	//XSSOK
...
<%
	       ...
	       if(!firstIndex.equals(""))
	       {
	           ...
	           if(calledMethod.equals("replaceExisting"))
	           {
	               ...
	               String strBlankEffectivityMesg = i18nStringNowUtil("emxUnresolvedEBOM.EBOM.BlankEffectivity.ReplaceOperation",
												                       "emxUnresolvedEBOMStringResource",
												                       language);
...
<%
                    ...
                    if ("false".equalsIgnoreCase(isWipMode)) {
                    	//boolReplaceOperValidation = unresolvedEBOM.emptyEffectivityReplaceValidation(context, selPartRelId, contextECO);
                    	boolReplaceOperValidation=false;
                    	
                    	if (boolReplaceOperValidation) {
		                    try {
...
%>
	                <script language="Javascript">
	                 ...
	                 //XSSOK
	                   ...
	                   else if(<%=boolReplaceOperValidation%> == false){
	                	 //XSSOK
	                       warningMessage = "<%=strBlankEffectivityMesg%>";
	                     }
	               </script>
	...
	//XSSOK
	    ...
	    //XSSOK
		 	...
		 	//var objWindow =  getTopWindow().modalDialog.contentWindow;
	                ...
	                contentURL = "<xss:encodeForURL><%=contentURL%></xss:encodeForURL>"+"&selectedPartsList="+selectedParts+"&frameName="+frameName;
				showModalDialog(contentURL);
		             }
              else if("addExisting" == "<%=XSSUtil.encodeForJavaScript(context,calledMethod)%>") {
            	  var is3DSearch = "<%=is3DSearch%>";
            	  var cFrame = findFrame(getTopWindow(),"PUEUEBOM");
            	  var tablemode = "";
            	  if(cFrame.editableTable && cFrame.editableTable != null && cFrame.editableTable != undefined){
            		  ...
            		  var selPartRowId = '<%=selPartRowId%>';
	      				...
	      				var nosRowsselected = "<%=nosRowsselected%>";
	 	      			...
	 	      			if(nosRowsselected>1){
			      			...
			      			contentURL = "<xss:encodeForURL><%=contentURL%></xss:encodeForURL>"+"&highestFN="+highest+"&submitURL=../unresolvedebom/AddExistingPartProcess.jsp?calledMethod="+calledMethod+"&tablemode="+tablemode+"&selPartRowId="+selPartRowId+"&frameName="+frameName+"&isWipMode="+<%=XSSUtil.encodeForJavaScript(context,isWipMode)%>;
			      			if( is3DSearch == "true"){
			      				showModalDialog(contentURL);		
			      			}
			      			else{
              ...
              //else if ('createAndAssignChangeOrder' == "<xss:encodeForJavaScript><%=calledMethod%></xss:encodeForJavaScript>" && isWipBomAllowed.toLowerCase()=="true"){
            	  ...
            	  if (selectedParts != "fromConfiguredViewMode") {
            	  	selectedParts = selectedParts != ""?selectedParts.substring(0,selectedParts.lastIndexOf('~')):"";
            	  }
            	  //XSSOK
            	  var targetURL = "<xss:encodeForURL><%=contentURL%></xss:encodeForURL>" + "&selectedPartsList=" + selectedParts + "&targetLocation=slidein";
