  ...
  { 
	...
	if("addToExistingPUEECO".equalsIgnoreCase(calledMethod)) {
		...
		ecoId  = (String) slECOObjectList.get(0);
			ecoId  = (String) slECOObjectList.get(0);
			ecoId = DomainObject.newInstance(context, ecoId).getId(context);
	}
...
<script language="javascript" src="../common/scripts/emxUICore.js"></script>
			...
			if(typeof getTopWindow().SnN != 'undefined' && getTopWindow().SnN.FROM_SNN){
						//emxUICore.findFrame(getTopWindow(),"PUEUEBOM").emxEditableTable.refreshStructureWithOutSort();
						emxUICore.findFrame(getTopWindow(),"<xss:encodeForJavaScript><%=frameName%></xss:encodeForJavaScript>").refreshSBTable();
				}
				else{
...
<%	
  ...
  if ("addExisting".equalsIgnoreCase(calledMethod)) {
  	...
  	int FNvalue = 1;
  	if(highestFN != null && !"NaN".equals(highestFN) && !("0".equals(highestFN))){
	  ...
	  //2012x -Ends
		  ...
		  for (int i=0; i < selectedItems.length ;i++)
		  {
			...
			if("view".equalsIgnoreCase(tablemode)){
		      ...
		      if ("true".equalsIgnoreCase(isWipMode)){
		    	  strInput = strInput + "<item oid=\""+ selectedId + "\" pid=\"" + selPartObjectId + "\" relType=\"relationship_EBOM\"><column name=\"CurrentEffectivity\"></column><column name=\"Quantity\" edited=\"true\">1.0</column><column name=\"Find Number\" edited=\"true\">"+ FNvalue +"</column><column name=\"UOM\" edited=\"true\">"+ sUOMForMarkUp +"</column><column name=\"VPMVisible\" edited=\"true\">True</column></item>";
		      }else{
...
<script language="Javascript">
//XSSOK
var isWipBomAllowed = "<xss:encodeForJavaScript><%=isWipBomAllowed%></xss:encodeForJavaScript>";
...
var frameName = "<xss:encodeForJavaScript><%=frameName%></xss:encodeForJavaScript>";
	...
	if(typeof getTopWindow().SnN != 'undefined' && getTopWindow().SnN.FROM_SNN || !getTopWindow().getWindowOpener()){
		...
		callback  = eval(emxUICore.findFrame(getTopWindow(),"<xss:encodeForJavaScript><%=frameName%></xss:encodeForJavaScript>").emxEditableTable.<%=callbackFunctionName%>);
	}
 		else{
	   ...
	   var frame = emxUICore.findFrame(getTopWindow(), "<xss:encodeForJavaScript><%=frameName%></xss:encodeForJavaScript>");
