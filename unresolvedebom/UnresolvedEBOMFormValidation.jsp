...
function submitFilterButton() {
	...
	var related = getTopWindow().document.getElementById("ENCPartWhereUsedRelated").value;
	if(related == "Specification")
		sURL += "&sortColumnName=none";
    ...
    for (var i = 0; i < fieldArr.length; i++) {
        ...
        if(fieldObject== null || fieldObject == "undefined" || fieldObject == "null" || fieldObject.value == "" || fieldObject.value == null){
		 ...
		 if (fieldArr[i] == "PUEUEBOMProductConfigurationFilter_actualValue" || fieldArr[i] == "PUEUEBOMProductConfigurationFilter") {	// Take product configuration actual from hidden parameters always        		
				var toolbarPCValue     = jQuery('input[id='+fieldArr[i]+']',jQuery("div#ENCPartWhereUsedFiltersToolbar2",getTopWindow().document));				
				fieldObject			   = (toolbarPCValue && toolbarPCValue.val() != null && toolbarPCValue.val() != "undefined" && toolbarPCValue.val() != "" ) ? toolbarPCValue : fieldObject ;
				
				if(fieldObject != null && fieldObject != "undefined" && fieldObject[0]){
        ...
        if (fieldObject != null && fieldObject != "undefined") {
            ...
            if (fieldArr[i] == "ENCPartWhereUsedRelated" && (fieldObject.value != "None")) {
                sURL += "&expandProgram=emxPart:getWhereUsedRelatedDatas&expandByDefault=true";
            }
        }
    }
...
function replaceWith(functionality) {
    ...
    if(functionality == "replaceWithSpecificRevision" || functionality == "replaceWithSpecificRevisionForAll") {
    	submitToFloatOnEBOM(functionality,"../engineeringcentral/ENGFloatOnEBOM.jsp");
    }	  	  		  	  	
	else 
	{	
...
function submitToFloatOnEBOM(functionalityValue,urlToSent) {
	 if(document.emxTableForm.functionality) {
	 	document.emxTableForm.functionality.value = functionalityValue;
	 }
	 else {
	 	document.emxTableForm.appendChild(getHiddenField("functionality",functionalityValue));
	 }	 	 	
  ...
  function submitFromRevisionManagement() {
	 ...
	 submitToFloatOnEBOM("submitFromRevisionManagement","../engineeringcentral/EBOMFloat.jsp");  	
