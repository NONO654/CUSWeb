    		...
    		<script language="javascript">
			   ...
			   if(null==detailsDisplay){
					...
					detailsDisplay.location.href = detailsDisplay.location.href;
						...
						WBSFrame.refreshStructureWithOutSort();
					}
				}else{
...
<%
  	...
  	String strURL = "../common/emxIndentedTable.jsp?table=PMCAddExternalTaskDependencyTable&freezePane=Title&selection=single&header=eServiceSuiteProgramCentral.ExternalCrossProjectDependencyStep1.heading&program=emxProjectSpace:getExternalProjects&HelpMarker=emxhelpprojectcopypage2&suiteKey=ProgramCentral&SuiteDirectory=programcentral&StringResourceFileId=emxProgramCentralStringResource&emxSuiteDirectory=programcentral";
  		...
  		strURL = strURL + "&rowGrouping=false&Export=false&multiColumnSort=false&showPageURLIcon=false&displayView=details&showClipboard=false&objectCompare=false&massUpdate=false&findMxLink=false&PrinterFriendly=false&autoFilter=false&triggerValidation=false&massPromoteDemote=false&externalDependency=true&submitLabel=emxProgramCentral.Button.Next&submitURL=../programcentral/emxProgramCentralUtil.jsp&mode=addExternalTasks";
...
<%
 		...
 		String strURL = "../common/emxFullSearch.jsp?field=TYPES=type_ProjectSpace:Type!=type_Experiment&table=PMCProjectSummaryForProjects&selection=multiple&toolbar=&editLink=true&submitURL=../programcentral/emxProgramCentralUtil.jsp&mode=addProjectToProgram";
	...
	<%
	    		...
	    		%>
	    			<script language="javascript">
	    				...
	    				var projectRole;
	    				var frame = emxUICore.findFrame(getTopWindow(), "slideInFrame");
	    				if(frame !=null){
	    					projectRole = frame.emxCreateForm.ProjectRoleId.value;
	    				} else {
	    				 projectRole = top.window.opener.emxCreateForm.ProjectRoleId.value;
	    				}
	    				var url = "<%=strURL%>";	<%-- XSSOK --%>
	    				url = url + "&SelectedProjectRole="+projectRole+"&frameNameForField="+parent.name;
	    				showModalDialog(url);
	    			</script>
	    		<%
				...
				while (itrNewTask.hasNext()) {
					...
					if("addTaskAbove".equalsIgnoreCase(mode)){
						System.out.println("----------------------addTaskAbove");
						...
						sBuff.append("<item oid=\"" + toId + "\" relId=\"" + relId + "\" pid=\"" + fromId + "\"  direction=\"" + "from" + "\" />");
						sBuff.append("</data>");
					}
				}
				sBuff.append("</mxRoot>");
				System.out.println("sBuff---------------------1----------------"+sBuff);
	    	  %>
	    	  <script language="javascript">
	    	  var frame = "<%=portalCommandName%>";
			  alert("frame--"+frame);
	    	  ...
	    	  var topFrame = findFrame(getTopWindow(), frame);
				 alert("topFrame--"+topFrame.location.href);
	    	  if(null == topFrame){
			...
			if(!topFrame.dataGridEnabled){
	    	  ...
	    	  <%
	    		...
	    		<%
                    ...
                    if(!existingCalenderNames.isEmpty())
				    {
				    %>
				    <script language="javascript" type="text/javaScript">
				    
         			alert("<framework:i18nScript localize="i18nId">emxProgramCentral.Calendar.AlreadyConnectedCalendar</framework:i18nScript>" +"<%= existingCalenderNames.toString()%>");
         			</script>
...
<%
	...
	if(i == 0){
	 ...
	 formURL="../common/emxForm.jsp?form=PMCProjectTemplateFolderContentReportForm&suiteKey=ProgramCentral&formHeader=emxProgramCentral.Common.FolderContentReport&selectedIds="+rowIds+"&objectId="+XSSUtil.encodeForURL(context, objectId)+"&HelpMarker=emxhelpfoldercontentreport";
		 }
		 else{
		 formURL="../common/emxForm.jsp?form=PMCProjectFolderContentReportForm&suiteKey=ProgramCentral&formHeader=emxProgramCentral.Common.FolderContentReport&selectedIds="+rowIds+"&objectId="+XSSUtil.encodeForURL(context, objectId)+"&HelpMarker=emxhelpfoldercontentreport";
		 }
...
<%
	...
	<%
		...
		String projectSchedule = projectScheduleMap.get(projectId);
     	...
     	Set<String> parentIdList = Task.taskIndentation(context, selectedTaskIds, action);
     	...
     	if(parentIdList.isEmpty()){
	    	isRefreshStructure = false;
     		...
     		BusinessObjectWithSelectList objectWithSelectList =
					ProgramCentralUtil.getObjectWithSelectList(context, parentObjIdArray, new StringList(ProgramCentralConstants.SELECT_ID),true);
     		for(int i=0;i<objectWithSelectList.size();i++){
     			BusinessObjectWithSelect bws = objectWithSelectList.getElement(i);
     			String parentObjId = bws.getSelectData(ProgramCentralConstants.SELECT_ID);
				parentObjIdList.add(parentObjId);
				 		    }
			...
			if(isRefresh == "true"){
			  ...
			  %>
				...
				if(topFrame.dataGridEnabled){
		            ...
		            if(topFrame.dataGridEnabled){
						topFrame.emxEditableTable.select(topFrame.emxEditableTable.getAttributeFromNode("rowId",selectdObjId));
							}else{
								...
								if("moveUp"==action){
									...
									abvRow.parentNode.insertBefore(childRows[index],childRows[index-1]);
									...
									abvRow.parentNode.insertBefore(childRows[index+1],childRows[index]);
									//abvRow.parentNode.setAttribute("display",'block');
								}
			...
			String strURL = "../common/emxIndentedTable.jsp?program=emxProjectMember:getObjectAccessList&table=PMCProjectMemberSummary&toolbar=PMCMemberSummaryToolBar&header=emxProgramCentral.Common.Members&HelpMarker=emxhelpprojectmembers&editRootNode=false&objectBased=false&hideRootSelection=true&displayView=details&showClipboard=false&HelpMarker=emxhelpmultipleownershipaccess&massUpdate=false&selection=multiple&Export=false&sortColumnName=Name&sortDirection=ascending&isViewMode=true&level=0&expandLevelFilter=true&freezePane=Project,Organization&massUpdate=true&persistent=false&showRMB=false";
          ...
          <%
			...
			return;
               ...
               String[] methodArgs = JPO.packArgs(paramMap);
               boolean hasModifyAccess  = (boolean) JPO.invoke(context,"emxProjectTemplate", null, "isOwnerOrCoOwner", methodArgs, Boolean.class);
       ...
       <%
      	  ...
      	  String strURL = "../common/emxIndentedTable.jsp?tableMenu=PMCFolderTableMenu&toolbar=PMCFolderSummaryToolBar&"+
		  				  ...
		  				  "&massPromoteDemote=false&postProcessJPO=emxTask:postProcessRefresh";
		String objectId = emxGetParameter(request, "objectId");
...
<%
    ...
    <%
		    ...
		    if(size <= 0 && (errorMsg == null || errorMsg=="")){
      		...
      		String strURL = "../common/emxForm.jsp?form=PMCProjectPreferences&mode=view&HelpMarker=emxhelpprojectdetails&Export=false";
		...
		<%
    	  ...
    	  <%
         	...
         	String strURL = "../common/emxIndentedTable.jsp?program=emxTask:getMembers&table=PMCTaskAssigneeSummary&selection=multiple&calculations=true&objectCompare=true&chart=true&toolbar=PMCAssigneeSummaryToolbar&header=emxProgramCentral.Common.TaskAssignees&HelpMarker=emxhelpassigneesummary&Export=false&sortColumnName=Name&sortDirection=ascending&preProcessJPO=emxTask:preProcessCheckForEdit&objectId="+objectId;

         	if(!DomainObject.STATE_PROJECT_SPACE_COMPLETE.equalsIgnoreCase(strState)){
         		strURL = strURL + "&editLink=true";
         	}
		...
		String sEnforceMandatoryTasks = EnoviaResourceBundle.getProperty(context,"emxProgramCentral.EnforceMandatoryTasks");
		...
		if("TRUE".equalsIgnoreCase(sEnforceMandatoryTasks)){
		...
		return;
            ...
            try {
                ...
                if (projectTemplates != null) {
                    ...
                    for (int i = 0; i < 1; i++) {
                        ...
                        if (!"Active".equalsIgnoreCase(boState)) {
		 ...
		 <%
            ...
            for(int i=0;i<emxTableRowId.length;i++)
            {
                ...
                if(!domainObject.isKindOf(context,com.matrixone.apps.common.CommonDocument.TYPE_DOCUMENTS))
                {
                    String sErrMsg = EnoviaResourceBundle.getProperty(context, "ProgramCentral",
                ...
                if(ProgramCentralUtil.isNotNullString(sObjectId)){
                       arrParsedDocObjIds[i] = sObjectId;
                }
             }
    		...
    		if(emxTableRowId.length <= 1){
		 	...
		 	String strURL = "../common/emxIndentedTable.jsp?table=PMCAddToFolder&program=emxProjectFolder:getProjectFolderList&suiteKey=ProgramCentral&header=emxProgramCentral.Common.SelectFolders&HelpMarker=emxhelpfolderselectdialog" +
		 					...
		 					"&autoFilter=false&showClipboard=false&showPageURLIcon=false&rowGrouping=false&customize=false&displayView=details&findMxLink=false&multiColumnSort=false&expandLevelFilter=true&submitLabel=emxFramework.Button.Submit";
			...
			for (int i = 0; numTasks > i; i++) {
				...
				try {
					...
					while (predecessorItr.hasNext()) {
						...
						if (predecessorId.equals(tasks[i])) {
