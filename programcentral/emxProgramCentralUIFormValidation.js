...
function validateSigma()
{
	...
	if(isNaN(Sigma.value)){
		var errorUrl = "../programcentral/emxProgramCentralUtil.jsp?mode=errorMessageWithPlaceHolder&msgValue=SigmaValue&errorKey=emxProgramCentral.Validate.ValidateValues";
		var errorMessage = emxUICore.getData(errorUrl);
		alert(errorMessage);
		return false;
	}
	else if (Sigma.value > 2147483647){
		var errorUrl = "../programcentral/emxProgramCentralUtil.jsp?mode=errorMessageWithPlaceHolder&msgValue=SigmaValue&errorKey=emxProgramCentral.Validate.ValueLessThan";
		var errorMessage = emxUICore.getData(errorUrl);
		alert(errorMessage);
		return false;
	}else{
...
function previewTemplateTasksWBS(){
	var href="../common/emxIndentedTable.jsp?table=PMCProjectSummaryForProjects&sortColumnName=taskId&program=emxProjectTemplate:getTemplateTasksWBS&expandProgram=emxProjectSpace:getTemplateWBS&suiteKey=ProgramCentral&SuiteDirectory=programcentral&StringResourceFileId=emxProgramCentralStringResource&emxSuiteDirectory=programcentral&freezePane=Name&hideHeader=true&customize=false&rowGrouping=false&showPageURLIcon=false&hideLaunchButton=true&export=false&displayView=details&export=false&hideRootSelection=true&objectCompare=false&showClipboard=false&multiColumnSort=false&findMxLink=false&showRMB=false&massPromoteDemote=false&expandLevelFilter=false";
	showModalDialog(href);
}
	...
	for (var key in responseJSONObj) {
		if(document.getElementsByName(key)[0]!= null && document.getElementsByName(key)[0] != 'undefined'){
			...
			if ((isMilestone || isGate) && "Duration" == key) {
				...
				document.getElementById("units_DurationId").disabled = true;
			}
		}
	}
...
function ChangeReviewTaskField() {
	...
	if (isMilestone || isGate) {
		...
		document.getElementById("units_DurationId").disabled = false;
	}
...
function validateNumberOfForFolder() {
	...
	function setScopListVal(){
		//var val = document.getElementsByName("name")[0].type;
