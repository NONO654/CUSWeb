 ...
 */
function submitQuickFilter()
{
  //get the url which have been used to display the table
  var tableDocument=(this.name && this.name=='listHead') ?  /* portal mode */ this.parent.document : /* content or popup mode */ document;
  var emxTableUrl=tableDocument.location.href;
  //treat inputs of the current document
  var inputs = document.querySelectorAll(".pageHead input[type='text'], .pageHead select");
  for (i = 0; i < inputs.length; i++) {
      var input = inputs[i];
      if (input.id !== null && input.id.length > 0) {
        emxTableUrl = delParameterFromUrl(emxTableUrl, input.id);
        if (input.value !== null && input.value.length > 0) {
          input.value = input.value.replace(/\|/g,"");
          emxTableUrl = addParameterToUrl(emxTableUrl, input.id, input.value);
        }
      }
    }
