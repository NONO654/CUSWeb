...
function processModalDialog(msfParam, strURL, intWidth, intHeight, bScrollbars, strPopupSize,searchDim) {
	var requireObj;
	try{
		if(require) {
			requireObj = require;
	...
	requireObj(["DS/MSFDocumentManagement/MSFDocumentClient"], function(MSFDocumentClient) {
		...
		if(MSFDocumentClient.isConnectedWithMSF() === "true") {
			MSFDocumentClient.sendMessage(msfParam);
		}
		else {
			showModalDialog(strURL, intWidth, intHeight, bScrollbars, strPopupSize, searchDim)
		}
	}, function() {
		showModalDialog(strURL, intWidth, intHeight, bScrollbars, strPopupSize, searchDim)
	});
...
function returnIfSnN (snnOrFTSURL, searchURL) {
	...
	if(strData && strData.indexOf("&source=") < 0){
		strData = strData + "&source=3dspace";
	}
	var searchData = emxUICore.getDataPost(strURL, strData);
	if((searchData.trim() != "") && JSON.parse(searchData).EXCEPTION != null){
		alert(JSON.parse(searchData).EXCEPTION);
		return true;
	}else{
		if(searchData.trim() != ""){
			if(getTopWindow().SnN){
                ...
                var searchTempURL = "../common/SearchUI.html";
				getTopWindow().searchInfo = searchData;
				getTopWindow().searchURL = snnOrFTSURL;
				
				if(searchURL.indexOf("addPreCondParam=false") > -1){
    				searchTempURL = searchTempURL + "?addPreCondParam=false";	
				}
				showModalDialog(searchTempURL, emxUIConstants.ARR_PopupWidth['Large'], emxUIConstants.ARR_PopupHeight['Large'], true, 'Large');
				return true;
			}
		}else{
			return false;
...
function closeAllChildWindows() {
    ...
    if (getTopWindow().childWindows && (typeof mmyReload == "undefined" || !mmyReload))
    {
...
function isTablePage(strURL){
	...
	var isDataGridTable = false;
	var index =  strURL.indexOf(".jsp");
	if (index < 0) {
	    index = strURL.indexOf(".html")
	}
	...
	if(subStr.indexOf("ENXSBGridConnectorClient") >= 0){
		isDataGridTable = true;
	}
	
	if(isTable || isIndentedTable || isGridTable || isDataGridTable){
		itp = true;
...
function openNavigatorDialog(strURL, strFeatures, isApplyIEMask){
	...
	var strChangedURL=strURL.replace("fromExt=true","");
	objWindow.name = objWindow.name + "||" + strChangedURL;
    	if(isApplyIEMask){
...
function updateShortcutMap(update, id){
	...
	$.ajax({
		   url: "emxShortcutGetData.jsp?action=updateShortcutInfo&key="+key+"&id="+encodeURIComponent(id)+"&update="+update,
		   cache: false
...
function showTransientMessage(msg, messageType, className, delay){
	var options={
			message     : msg,
...
function showGenericTransientMessage(options) {
	...
	if(! options.customClass){
		options.customClass = "alert-right";
	}
	require(['DS/UIKIT/Alert'], function (Alert) {
		 ...
		 myAlert.elements.container.addClassName(options.customClass);
         if(options.events){
