...
var objDefaultCategory = "<%=XSSUtil.encodeForJavaScript(context,defaultCategory)%>";
...
getTopWindow().loadBreadCrumbPage("<%=XSSUtil.encodeForURL(context, objectId)%>", "<%=XSSUtil.encodeForURL(context, treelabel)%>", ":", "<%=XSSUtil.encodeForJavaScript(context, bcLabel)%>", url, "<%=treeNodeUrl%>", null, "<%=XSSUtil.decodeFromURL(treelabel)%>", catObj,jsonOpts,appInfoJSONObj);
    ...
    <%} else {%>
		if(getTopWindow().bclist.getCurrentBC() && getTopWindow().bclist.getCurrentBC().id == currObjId) {
			getTopWindow().loadBreadCrumbPage("<%=XSSUtil.encodeForURL(context, objectId)%>", "<%=XSSUtil.encodeForURL(context, treelabel)%>", ":", "<%=XSSUtil.encodeForJavaScript(context, bcLabel)%>", "<%=treeDisplayURL.toString()%>", "<%=treeNodeUrl%>", true, "<%=XSSUtil.decodeFromURL(treelabel)%>", catObj,jsonOpts);
		} else {
    getTopWindow().loadBreadCrumbPage("<%=XSSUtil.encodeForURL(context, objectId)%>", "<%=XSSUtil.encodeForURL(context, treelabel)%>", ":", "<%=XSSUtil.encodeForJavaScript(context, bcLabel)%>", "<%=treeDisplayURL.toString()%>", "<%=treeNodeUrl%>", null, "<%=XSSUtil.decodeFromURL(treelabel)%>", catObj,jsonOpts,appInfoJSONObj);
		}
    <%}%>       
<%}%>
