<%--  smaExeJobUTil.jsp   - jsp that calls the job related operations  
 * 
...
<%
    ...
    if ( (objectAction == null) || (objectAction.equals("")) )
    {
        emxNavErrorObject.addMessage(
            "smaExeJobUtil.jsp objectAction not found");
        objectAction = "";
    }
    ...
    if(objectAction.equals("pause")){
        jobId = simJob.getId(context);
        if(jobId!=null)
        {
            ...
            URL = URL+"/execution/"+eedJobId+"/pause";
    ...
    else if(objectAction.equals("continue")){
        jobId = simJob.getId(context);
        if(jobId!=null)
        {
            String [] jpoArgs = {jobId,"MODIFY" };
            ...
            String eedJobId = (String)eedInfo.get("eedJobId");
            String URL = (String)eedInfo.get("eedURL"); 
            URL = URL+"/execution/"+eedJobId+"/resume";
            String ticket = (String)eedInfo.get("ticket"); 
         // javascript for calling webservices
            %>
            <script language = javascript>
...
function refreshCallerFrame (submitAction, refreshFrame)
{
    ...
    // considering Pause/Continue Command is only resides in Jobs Page
