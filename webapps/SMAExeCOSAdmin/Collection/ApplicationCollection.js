/*
* Collection for applications the url is the url for the station
* it will get the application data from the application list that is returned as part of the station info
*/
define('DS/SMAExeCOSAdmin/Collection/ApplicationCollection', [
    'UWA/Core',
...
], function (UWA, Collection, Promise, ApplicationModel, WSUtils, WebappsUtils) {
    'use strict';
    return Collection.extend({
        ...
        parse: function (data) {
        	...
        	if (myStation){
        		if (myStation.ApplicationList){
        			if (myStation.ApplicationList.Application.length){
        				myStation.ApplicationList.Application.forEach(function (app) {
        					appArray.push(app);
                    	});
        			}
        			else {
        				appArray.push(myStation.ApplicationList.Application);
        			}
        		}
        	}
        	appArray.forEach(function (item) {
        		...
        		item.path = item['@ExecPath'];
        		item.id = item.name + '___' + item.version;
        	});
    		return appArray;
        }
    });
