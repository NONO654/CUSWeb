...
(function (window) {
	...
	window.Polymer({
		...
		attached: function () {
			...
			this.fire('xs-wg-section-attached');
		},
		...
		onConfirmRemove: function (event) {
			...
			if (childPresent && rulesPresent){
				this.fire('confirmDelete', {
					item: this,
					method: 'onRemove',
					msg: 'This will delete all the components inside the Section and make some rules unusable. Do you want to delete?'
				});
			} else if (childPresent) {
				this.fire('confirmDelete', {
					item: this,
					method: 'onRemove',
					msg: 'This will delete all the components inside the Section. Do you want to delete?'
				});
			} else {
				this.onRemove();
			}
		...
		onRemove: function () {
			...
			items.forEach(function (item) {
				// Check that related xs-wg-plm-data is also removed
		...
		/**
		 * Checkes wheather component is to dropped after current component or not
		 * @param {Element} item xs-wg-* element which is dropped
		 * @param {Element} refItem reference item on which item is dropped
		 * @returns {Boolean} if it has to be inserted after then true else false
		 */
		...
		// Is this component draggable or not, if disbaled then it should not be draggable.
		// In case component is in any mode other than design it should not be draggable
		...
		// Hides the component completely if hidden property is set to true
		_computeMainClass: function (label, isDesignMode, _isTopLayout, hidden) {
			...
			if (_isTopLayout || !isDesignMode) {
				return 'layout_main_container';
			}
			return 'layout_container';
		},

		// hide handle if its a toploayout or if its not in design mode and not in design mode
		...
		/**
		* compute class of body based on this component is in collapsed state or not - to collapse component
		* @param {Boolean} iscollapsible is this component collapsible or not
		* @param {Boolean} collapsed is this component collapsed or not
		* @param {Boolean} _isTopLayout is this top loyaout
		* @returns {String} class for body in case collapse property is set or not
		*/
		_computeClassBody: function (iscollapsible, collapsed, _isTopLayout) {
		...
		/**
		* computes class based on column - user defined and maximum column allowed - for column number 1
		* @param {Number} vertical property set by user on number of column
		* @param {Number} _maxColumnLevel Maximum column that can be rendered as per width available
		* @param {Boolean} _isTopLayout is this top loyaout
		* @returns {String} class to identify width of column in section
		*/
		...
		/**
		* computes class based on column - user defined and maximum column allowed - for column number 2
		* @param {Number} vertical property set by user on number of column
		* @param {Number} _maxColumnLevel Maximum column that can be rendered as per width available
		* @returns {String} class to identify width of column in section
		*/
		...
		/**
		* computes class based on column - user defined and maximum column allowed - for column number 3
		* @param {Number} vertical property set by user on number of column
		* @param {Number} _maxColumnLevel Maximum column that can be rendered as per width available
		* @returns {String} class to identify width of column in section
		*/
		...
		/**
		 * Computes class to show or hide delete icon
		 * @param {Boolean} _hover When mouse is hovered on handle bar
		 * @param {Boolean} disabled is component disabled
		 * @returns {String} class to show or hide delete icon
		 */
