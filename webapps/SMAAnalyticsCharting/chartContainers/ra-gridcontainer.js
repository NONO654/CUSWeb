(function(GLOBAL) {
    ...
    gridContainerPrototype.createdCallback = function() {
    ...
    gridContainerPrototype.getGridFrame = function(args) {
        var that = this;
        var el = document.createElement('ra-gridframe');
    ...
    gridContainerPrototype.focusFrame = function(focusFrame) {
        var frames = this.querySelectorAll('ra-gridframe');
    ...
    gridContainerPrototype.getChartConfig = function(chartId) {
    ...
    gridContainerPrototype.setSingleMode = function(singleMode) {
        this._singleMode = singleMode;
    };
    gridContainerPrototype.restoreChart = function(chart, index, cb) {
        var that = this;
        var chartUrl;
        ...
        var frame = that.getGridFrame(defaultFrameOptions);
        var frameContent = frame.querySelector('.content');
        frameContent.controls = frameContent.controls || {};
        that.propagateChartEvent({ eventType: 'mask' }, [frameContent]);
        Polymer.Base.importHref(chartUrl, function() {
    ...
    gridContainerPrototype.createNewChartDialog = function() {
        // Create a mobile dialog that can be used to generate a new chart.
    };

    gridContainerPrototype.createNewChart = function() {
        // Create a chart element and append it to the parent
    };
    
    // FIXME: IS this the right place to track this information?
    // sd4: store key press information
    gridContainerPrototype.getKeyPress = function() {
        return gridContainerPrototype.keyPress;
    };
    ...
    window.addEventListener(
        ...
        function(event) {
            ...
            if (vKey === 17) {
                gridContainerPrototype.keyPress.ctrl = true;
            } else if (vKey === 13) {
                gridContainerPrototype.keyPress.shift = true;
            } else if (vKey === 18) {
                gridContainerPrototype.keyPress.alt = true;
            }
        },
        false
    );
    ...
    window.addEventListener(
        ...
        function(event) {
            var vKey = event.keyCode
                ...
                : event.which ? event.which : event.charCode;
            if (vKey === 17) {
                gridContainerPrototype.keyPress.ctrl = false;
            } else if (vKey === 13) {
                gridContainerPrototype.keyPress.shift = false;
            } else if (vKey === 18) {
                gridContainerPrototype.keyPress.alt = false;
            }
        },
        false
    );
    
    Polymer(gridContainerPrototype);
    GLOBAL.DS.RAComponents.gridcontainer = gridContainerPrototype;
})(this);
