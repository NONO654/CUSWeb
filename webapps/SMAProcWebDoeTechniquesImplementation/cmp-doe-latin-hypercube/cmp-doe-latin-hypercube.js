/**
 * @class element:cmp-doe-latin-hypercube
 * @noinstancector
 * @description
 * This component implements the Latin Hypercube technique in DOE.
 * The Latin Hypercube technique is a class of experimental designs that efficiently samples large design spaces.
 */

require(['SMAProcWebDoeTechniquesBase/SMAProcDesignDriverFactorImpl'],
function(SMAProcDesignDriverFactorImpl){
    'use strict';
    window.Polymer(/** @lends element:cmp-doe-latin-hypercube# */{
        is: 'cmp-doe-latin-hypercube',

        properties: /** @lends element:cmp-doe-latin-hypercube# */{
        ...
        Apply: function (pluginExtensionConfig) {
            console.log('In Apply() of cmp-doe-latin-hypercube..');
        ...
        populateDefaults: function(cellinfo, paramName) {
            ...
            if (SMAProcDesignDriverFactorImpl.findFactor(paramName, this.updatedFactors) < 0) {
                ...
                for (var j = 0; j < this.factorAttributes.length; j++){
                    ...
                    if (this.factorAttributes[j] === 'Baseline' && (cellinfo.paramvalue !== null && cellinfo.paramvalue !== undefined)) {
                        factor[this.factorAttributes[j]] = cellinfo.paramvalue;
                    }
                }
        ...
        populateFactorRow: function(factor) {
            ...
            if(cellinfo.grid) {
               for (var i=0; i<this.factorAttributes.length; i++) {
                   var attribute = this.factorAttributes[i];
                   cellinfo.grid[attribute] = factor[attribute].toString();
               } 
            } else {
                //title is set for tool tip
        ...
        updateFactorInfo: function (cellinfo, paramName) {
            ...
            var attributeToUpdate = this.updatedFactors[index][cellinfo.dataIndex];
            if (attributeToUpdate) {
                attributeToUpdate = cellinfo.nodeModel._options.grid[cellinfo.dataIndex];
