...
String rdUniquenessForConfigured = "";
if(isECCInstalled){
	rdUniquenessForConfigured = JSPUtil.getCentralProperty(application, session,"emxUnresolvedEBOM","ReferenceDesignatorUnique");
}
boolean isAutoCollaborationEnabled = iParameterization.isAutoCollaborationEnabled(context);
//tjx
...
String quantityCanNotBeModified = EnoviaResourceBundle.getProperty(context,propertySuite,locale,"emxEngineeringCentral.validate.QuantityCanNotBeModified");
String quantityValueinInstanceModeforAdd = EnoviaResourceBundle.getProperty(context,propertySuite,locale,"emxEngineeringCentral.validate.quantityValueinInstanceModeforAdd");
String sUOMCanNotBeModified = EnoviaResourceBundle.getProperty(context,propertySuite,locale,"emxEngineeringCentral.validate.UOMCanNotBeModified");
...
String selectWorkUnderChange = EnoviaResourceBundle.getProperty(context, propertySuite, locale, "emxEngineeringCentral.command.SelectWorkUnderChange");
String workUnderChange = EnoviaResourceBundle.getProperty(context, propertySuite, locale, "emxEngineeringCentral.BOM.WorkUnderChange");
...
String strEditInConfigContext = EnoviaResourceBundle.getProperty(context, propertySuite,context.getLocale(),"emxEngineeringCentral.Command.editPartsInViewModeForConfiguredPart");
...
// For Auto Sync
String strQtyvaluewithAutoCollab = EnoviaResourceBundle.getProperty(context, propertySuite,context.getLocale(),"emxEngineeringCentral.validate.quantityValueinQtyModeWithAutoCollabarotion");
...
<%@include file = "emx3DPlayCrossHighlightJavaScript.inc" %>


//Added for IR-464724-3DEXPERIENCER2017x-start
if(window.parent.subsObj == null||typeof window.parent.subsObj==undefined){
    		window.parent.subsObj = new Object();
    		
    	}else {
    		selectCB.onAddToken = window.parent.subsObj.selectCBAddToken;
    		selectCB.onRemoveToken = window.parent.subsObj.selectCBRemoveToken;
    		
    	}
  	
    	if(window.parent.compare3D===true) {
    		subscribe(window.parent.viewer1, false);
    		subscribe(window.parent.viewer2, false);
    	}else {
    		subscribe(window.parent.viewer, false);    		
    	}
//Added for IR-464724-3DEXPERIENCER2017x-end
//Modified for IR-462286-3DEXPERIENCER2017x -IE Issue

window.parent.parent.$(window.parent.parent.document).trigger("BOMREADY",{'subscribe':subscribe});
...
//XSSOK
var rdUniquenessForConfigured           = "<%=rdUniquenessForConfigured%>";
//XSSOK
...
var mQty= colMap.getColumnByName("Quantity");
var mUOM= colMap.getColumnByName("UOM");
var mState= colMap.getColumnByName("State");
var mChangeControlled = colMap.getColumnByName("ChangeControlled");
var mPolicy = colMap.getColumnByName("Policy");
var isENCBOM = this.location.href.indexOf("&table=ENCEBOMIndentedSummary") > -1
var maxQtyinQtyMode = 100;


if (isENCBOM && (mRD == undefined || mFN == undefined || mQty == undefined || mUOM == undefined || mType == undefined || mName == undefined || mRev == undefined || mState == undefined || mChangeControlled == undefined || mPolicy == undefined)) {
...
function isFNUniqueOnInstance(inputFN,level,curobjId,parentObjId,curUOM,VPMVisible,rdValue,rowid){

   	   //check if parts exists with inputFN
   	   var sameObjectRows = emxUICore.selectNodes(oXML,"/mxRoot/rows//r[child::c[@a='"+inputFN+"'] and @level = '" + level + "' and @id != '" + rowid + "']");
   	   var addModifiedObj =emxUICore.selectNodes(oXML, "/mxRoot/rows//r[(@status='add' or @status='changed') and @level = '" + level + "' and @id != '" + rowid + "']");
   	   var sameObjectFinalArr=[];
   	   var status = currentRow.getAttribute("status");
   	   
   	   //since add and changed rows are not captured ,capture same object ids
   	   if(addModifiedObj && addModifiedObj.length>0){
	   	   	for(var i=0;i<addModifiedObj.length;i++){
	   	   			if(sameObjectRows.indexOf(addModifiedObj[i]) == -1){
	   	   				sameObjectRows.push(addModifiedObj[i]);
	   	   			}
	   	   		}		
   	   }
   	   ...
   	   if (sameObjectRows && sameObjectRows.length > 0) {
  			...
  			//first iterate to get same objects having fn same check and filter sameObjectRows array
  			for(var i=0; i<sameObjectRows.length; i++) { 
  				var existingFN=getCurrentValue("Find Number",sameObjectRows[i]);
  				var sameObjId=sameObjectRows[i].getAttribute('o');
  				if(existingFN == inputFN && 'cut' != sameObjectRows[i].getAttribute("status") && parentObjId == sameObjectRows[i].getAttribute('p')){
  					sameObjectFinalArr.push(sameObjectRows[i]);
  				}
  				if('changed' == sameObjectRows[i].getAttribute("status") && parentObjId == sameObjectRows[i].getAttribute('p')){
  					
  					 var modifiedRowFN=getCurrentValue("Find Number",sameObjectRows[i]);
	   	   			 var modifiedRowRD=getCurrentValue("Reference Designator",sameObjectRows[i]);
	   	   			 var modifiedRowID=sameObjectRows[i].getAttribute('id');
	   	   			 //var sameFNRows = emxUICore.selectNodes(oXML,"/mxRoot/rows//r[child::c[@a='"+modifiedRowFN+"'] and @level = '" + level + "' and @id != '" + modifiedRowID + "']");
					 var sameFNRows = emxUICore.selectNodes(oXML,"/mxRoot/rows//r[child::c[@newA='"+modifiedRowFN+"'] and @level = '" + level + "' and @id != '" + modifiedRowID + "']");
	   	   				 if(modifiedRowRD != "" || sameFNRows.length> 0){
	   	   			 		
	   	   			 	 	if (sameFNRows && sameFNRows.length > 0) {
	   	   			 	 		for(var j=0; j<sameFNRows.length; j++) { 
		   	   			 	 		if(sameObjectFinalArr.indexOf(sameFNRows[j]) == -1){
	  									sameObjectFinalArr.push(sameFNRows[j]);
	  								}
  								}
	   	   			 	 	}
	   	   			 	 }
  		 ...
  		 if (sameObjectFinalArr && sameObjectFinalArr.length > 0) {
  			 
	  		for(var i=0; i<sameObjectFinalArr.length; i++) { 
	  				var rowObjId=sameObjectFinalArr[i].getAttribute('o');
	  				var rowUOM=getActualValueForOtherColumn("UOM",sameObjectFinalArr[i]);
	  				if(rowUOM == "" || rowUOM == null){
	  					rowUOM =getCurrentValue("UOM",sameObjectFinalArr[i]);
	  				}
		  			var rowVPLMVisible=getCurrentValue("VPMVisible",sameObjectFinalArr[i]);
		  			var rowRDValue=getCurrentValue("Reference Designator",sameObjectFinalArr[i]);
		  			
		  				//if current obj is not equal to the rowobj and curent uom is eacheach and fnArray include inputfn
			  				if(rowObjId!=curobjId ||curUOM != "EA (each)" || curUOM != rowUOM){	  					
	  							return false;
				  			}
				  			if(rowVPLMVisible != VPMVisible){
	  							return false;
	  						}
	  						if((rowRDValue!="" && rowRDValue!=" " && rdValue=="")){
	  							return false;
	  						}
	  						if((rowRDValue=="" && rdValue=="")){
  								continue;
  							}
			  				if((rowRDValue == "")){
				  				return false;
...
function isFNUniqueOnInstanceWithAlertMsgOnFailure(inputFN,level,curobjId,parentObjId,curUOM,VPMVisible,rdValue,rowid){
  		 ...
  		 if (sameObjectFinalArr && sameObjectFinalArr.length > 0) {
	  		...
	  		for(var i=0; i<sameObjectFinalArr.length; i++) { 
		  			...
		  			var rowRDValue=getCurrentValue("Reference Designator",sameObjectFinalArr[i]);
		  				...
		  				if(rowObjId!=curobjId){
                                alert("<emxUtil:i18nScript localize="i18nId">emxEngineeringCentral.findNumber.NonUniqueFindNumberNotAllowed</emxUtil:i18nScript>");
                                return false;
                            }
		  				
			  				if(curUOM != "EA (each)" || curUOM != rowUOM){
			  				    alert("<emxUtil:i18nScript localize="i18nId">emxEngineeringCentral.findNumber.UOMMustBeUnique</emxUtil:i18nScript>");	  					
	  							return false;
				  			}
				  			if(rowVPLMVisible != VPMVisible){
				  			
				  				alert("<emxUtil:i18nScript localize="i18nId">emxEngineeringCentral.findNumber.DesignCollaborationMustbeSame</emxUtil:i18nScript>");
	  							return false;
	  						}
	  						if ( (rowRDValue!="" && rowRDValue!=" " && rdValue=="") || (rowRDValue=="" && rdValue!="") ) {
	  							alert("<emxUtil:i18nScript localize="i18nId">emxEngineeringCentral.findNumber.ReferenceDesignatorError</emxUtil:i18nScript>");
	  							return false;
	  						}
	  						if((rowRDValue=="" && rdValue=="")){
  								continue;
...
function validateVPMVisibleOnApply(){

	var alevel = currentRow.getAttribute("level");
    var currentobjId = currentRow.getAttribute("o");
    var curRowId = currentRow.getAttribute("id");
    var parentRowID = this.currentRow.parentNode != null?this.currentRow.parentNode.getAttribute("id"):"";
    var parentObjectID = this.currentRow.parentNode != null?this.currentRow.parentNode.getAttribute("o"):"";
	var parentpolicyAttrib = parentRowID != ""?emxEditableTable.getCellValueByRowId(parentRowID,"Policy"):"";
	var parentpolicy = parentRowID != ""?parentpolicyAttrib.value.current.actual:"";
    var currentUOMValue = emxEditableTable.getCellValueByRowId(this.currentRow.getAttribute("id"),"UOM").value.current.actual;
    var curFN = emxEditableTable.getCellValueByRowId(this.currentRow.getAttribute("id"),"Find Number").value.current.actual;
	var VPMVisibleValue = getVPMVisibleValue();
    var RDValue = emxEditableTable.getCellValueByRowId(this.currentRow.getAttribute("id"),"Reference Designator").value.current.actual;
    var sUOMValue    = arguments[0];
    if(parentpolicy !="<%=EngineeringConstants.POLICY_CONFIGURED_PART%>"){
		if(!isFNUniqueOnInstance(curFN,alevel,currentobjId,parentObjectID,currentUOMValue,VPMVisibleValue,RDValue,curRowId)){
		    return "<emxUtil:i18nScript localize="i18nId">emxEngineeringCentral.findNumber.DesignCollaborationMustbeSame</emxUtil:i18nScript>";
		 }
	 }
	 return "";
}
...
//R208.HF1 - Ends
{
    ...
    var RDValue = emxEditableTable.getCellValueByRowId(this.currentRow.getAttribute("id"),"Reference Designator").value.current.actual;
	    ...
	    if("<%=isInstaceMode%>" == "true"){
	    	var currentQuantityValue = emxEditableTable.getCellValueByRowId(this.currentRow.getAttribute("id"),"Quantity").value.current.actual;
	    	if(currentUOMValue ==  "EA (each)" && currentQuantityValue % 1 != 0 && currentQuantityValue != 1)
	    	{
	    		 alert("<emxUtil:i18nScript localize="i18nId">emxEngineeringCentral.InstanceMode.NoModifyAccessOnLegacyComponents</emxUtil:i18nScript>");
	    		 return false;
	    	}
    	}
    }
    var currentObjRDValue = getValueForColumn(attRefDes);
    var currentObjID = currentRow.getAttribute('o');
...
<!-- XSSOK -->
	if(parentpolicy!="<%=EngineeringConstants.POLICY_CONFIGURED_PART%>"){
		if(!isFNUniqueOnInstanceWithAlertMsgOnFailure(inputFN,alevel,currentobjId,parentObjectID,currentUOMValue,VPMVisibleValue,RDValue,rowId)){
				return false;
		}
	}
...
//R208.HF1 - Ends
{
	...
	var status = currentRow.getAttribute("status");
		if(status != "add" && status != "new"){
	    	var currentUOMValue = emxEditableTable.getCellValueByRowId(this.currentRow.getAttribute("id"),"UOM").value.current.actual;
	    	var currentQuantityValue = emxEditableTable.getCellValueByRowId(this.currentRow.getAttribute("id"),"Quantity").value.current.actual;
	    	if(currentUOMValue ==  "EA (each)" && currentQuantityValue != "1.0")
	    	{
	    		 alert("<emxUtil:i18nScript localize="i18nId">emxEngineeringCentral.InstanceMode.NoModifyAccessOnLegacyComponents</emxUtil:i18nScript>");
	    		 return false;
	    	}
	...
	else
	{
     ...
     if(inputRD!="")
     {
         ...
         {
			if(parentpolicy != "<%= POLICY_CONFIGURED_PART %>" || (rdUniquenessForConfigured && rdUniquenessForConfigured.toLowerCase()=="true")){
...
//R208.HF1 - Ends
{
    ...
    <!-- XSSOk -->
		if(rdUOM == "<%=rangeEAeach%>"){
			if(checkForDecimal(qtyvalue)) {
				  alert(VPLM_EA_EACH_STRING);
				  return false;
			  }
			  <!-- XSSOK -->
		 	if("<%=isInstaceMode%>" == "true" || parentpolicy =="<%=EngineeringConstants.POLICY_CONFIGURED_PART%>"){
		 		if((status == "add" || status == "new") && parseInt(qtyvalue)>100){
		 			<!-- XSSOK -->
		 			 alert ("<%=quantityValueinInstanceModeforAdd %>");
		                 return false;
		 		}
		     		<!-- XSSOK -->
		     		if (status != "add" && status != "new" && parseInt(qtyvalue)>1 && rdUOM == "<%=rangeEAeach%>") {
		                <!-- XSSOK -->
    ...
    if(qtyvalue != null)
    {
     ...
     if(qtyvalue == '0' || qtyvalue == '0.0' || qtyvalue == '+0' || qtyvalue1 == '0.0')
	  ...
	  // #IR017352
      if(rdUOM != undefined){
      if((rdUOM.slice(0, 2) == "EA") && (VPLM_EA_EACH == "false")){
      if(checkForDecimal(qtyvalue)) {
	      alert(VPLM_EA_EACH_STRING);
      return false;
      }
...
function validateFNByLevelOnApply(str1,str2)
    ...
    //R208.HF1 - Ends
    	...
    	var currentobjId = currentRow.getAttribute("o");
  	    var currentUOMValue = emxEditableTable.getCellValueByRowId(this.currentRow.getAttribute("id"),"UOM").value.current.actual;
   	    var inputFN = emxEditableTable.getCellValueByRowId(this.currentRow.getAttribute("id"),"Find Number").value.current.actual;
   		var VPMVisibleValue = getVPMVisibleValue();
   		var RDValue = emxEditableTable.getCellValueByRowId(this.currentRow.getAttribute("id"),"Reference Designator").value.current.actual;
		...
		for(var j=0;j<inputParentRowFNArray.length;j++) {
			...
			for(var i=0;i<inputFNArray.length;i++) {
				...
				//IR-028660V6R2011
				if(curFNVal.contains(",")){
					var fnValuesSingleRow = curFNVal.split(",");
					for(var l=0;l<fnValuesSingleRow.length;l++) {
							if(status !='cut'){
								inputFNValues[k] = fnValuesSingleRow[l];
								k++;
							}
							var strMsg = validateFNValue(curFNObj,fnValuesSingleRow[l])
			...
			<!-- XSSOk -->
			if(parentpolicy !="<%=EngineeringConstants.POLICY_CONFIGURED_PART%>"){
				if(!isFNUniqueOnInstance(inputFN,alevel,currentobjId,parentObjectID,currentUOMValue,VPMVisibleValue,RDValue,rowId)){
					return ("<%=EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource", context.getLocale(),"emxEngineeringCentral.findNumber.NonUniqueFindNumberNotAllowed")%>");
		     	}
		     }
				if(strMsg != "" && strMsg.length >0){
					return strMsg;
	...
	function validateRDByLevelOnApply()
	{
    	...
    	var currentobjId = currentRow.getAttribute("o");
     	var curRowId = currentRow.getAttribute("id");
	  	...
	  	if(parentpolicy !="<%=EngineeringConstants.POLICY_CONFIGURED_PART%>"){
		  	 if(!isFNUniqueOnInstance(curFN,alevel,currentobjId,parentObjectID,currentUOMValue,VPMVisibleValue,RDValue,curRowId)){
		    	return "<emxUtil:i18nScript localize="i18nId">emxEngineeringCentral.findNumber.ReferenceDesignatorError</emxUtil:i18nScript>";
			 }
		 }
		...
		for(var j=0;j<inputParentRowRDArray.length;j++) {
			...
			for(var i=0;i<inputRDArray.length;i++) {
				...
				curQtyVal = getCurrentValue("Quantity",curRDObj);
				//IR-028660V6R2011
				...
				strMsg = validateRDValue(curRDObj,trim(curRDVal));
				if(strMsg != "" && strMsg.length >0){
					return strMsg;
				}
...
sURL = sURL + "&portalMode="+modePortal;
    ...
    for (var i = 0; i < fieldArr.length; i++) {
        ...
        if (fieldObject != null && fieldObject != "undefined") {
            ...
            } else if(fieldArr[i] == "CFFExpressionFilterInput" || fieldArr[i] == "CFFExpressionFilterInput_actualValue"){
            	sURL += "&" + fieldArr[i] + "=" + encodeURIComponent(fieldObject.value);
            } else {
...
function crossHighlightENG(rowIds, flag) {
	...
	if(channel3dLiveFrame == null)
		channel3dLiveFrame 	= findFrame(getTopWindow(),"ENGLaunch3DLiveChannel");
...
channel3dPlayFrame1 = null;
channel3dPlayFrame = null;
var sbSelected = [];
//Function to select/deselect the highlights in ENG BOM Powerview 
//OOTB channel is used for 3DPlay 
function crossHighlightENG3DPlay(rowIds, flag) {
	var chkAllFlag = false;
	...
	if(channel3dPlayFrame == null)
		channel3dPlayFrame 	= findFrame(getTopWindow(),"ENGLaunch3DPlayChannel");
		var viewer=channel3dPlayFrame.viewer;
	...
	if(typeof channel3dPlayFrame != 'undefined' && channel3dPlayFrame != null) {
		...
		for (var j = 0; j < strIDs.length; j++) {
			...
			idPath = getIDPathFromXMLNode3DPlay(rowNode);
			if(idPath != null && idPath != ""){				
			if(flag){
				for(var k=0;k<idPath.length;k++){
				   sbSelected.push(idPath[k]);
				}
			}else {
			for(var n=0;n<idPath.length;n++){
			var found = $.inArray(idPath, sbSelected) > -1;
			var index = sbSelected.indexOf(idPath[n]);
				if (index > -1) {
		...
		try {
				if(getCurrentValue("Quantity",rowNode) != null) {
					var quan = getCurrentValue("Quantity",rowNode);
					if(parseInt(quan)>1){
						for(var count = 0; count < parseInt(quan); count++) {						 
						 subscribe(viewer, false);				
						 var id =[];
						 id.push(idPath[count]);
					     toggleSelect3DPlay(viewer, flag, id); 
					     subscribe(viewer, true);				
					}
					
					}else {
						subscribe(viewer, false);
						toggleSelect3DPlay(viewer, flag, idPath);		
						subscribe(viewer, true);			
					}								
				}
			} catch(e){}
			...
			require(['DS/UIKIT/Alert'], function (Alert) {
									...
									myAlert.add({ className: "error", message: "Product structure not synched!" });
								});
								return;
			}
		}	
...
//Function to highlight 3d play remaining nodes if qty > 1
function getAllNodes3DPlay(rowNode, idPath, index,viewer1,viewer2,mode) {
	...
	try {
	    if(getCurrentValue("Quantity", rowNode) != null) {
			...
			if(parseInt(quan)>1){
			for(var count = 0; count < parseInt(quan); count++) {
				subscribe(viewer1, false);
				subscribe(viewer2, false);
				 var id =[];
				 id.push(idPath[count]);
				 toggleSelect3DPlay(viewer1, mode, id);
				 subscribe(viewer1, true);
				 subscribe(viewer2, true);
			}
			}
		}
	} catch(e) {
...
//Function to highlight Compare results for 3d play
function highlightCompareItem3dPlay(rowIds, mode) {

var finalResultsFrame1="";
	if(finalResultsFrame1 == "") {
		finalResultsFrame1 = getFinalSBFrame3DPlay();
	}
	...
	for (var j = 0; j < strIDs.length; j++) {
		...
		if(channel3dPlayFrame == null || channel3dPlayFrame.location == null) {
			channel3dPlayFrame 	= findFrame(getTopWindow(),"ENGLaunch3DPlayChannel");
			channel3dPlayFrame1	= findFrame(getTopWindow(),"ENGLaunch3DPlayCompareSecondFrame");
		}
		...
		for (var j = 0; j < strIDs.length; j++) {
			aId = strIDs[j].split("|");
		    rowId = aId[3];
		//var enc =findFrame(getTopWindow(), finalResultsFrame);
		var enc =finalResultsFrame1;
		rowNode = emxUICore.selectSingleNode(enc.oXML, "/mxRoot/rows//r[@id = '" + rowId + "']");
		matchresult = rowNode.getAttribute("matchresult");		
		//idPath = getIDPathFromXMLNode3DPlay(rowNode, matchresult);
		idPath = getIDPathFromXMLNode3DPlayBOMCompare(rowNode, matchresult);
		if(mode){
			for(k=0;k<idPath.length;k++){
				   sbSelected.push(idPath[k]);
				}
			}else {
			for(var n=0;n<idPath.length;n++){
		...
		if(idPath == null || idPath == ""){
			require(['DS/UIKIT/Alert'], function (Alert) {
									var myAlert = new Alert({
										...
										visible: true,
										autoHide: true
									}).inject(window.document.body, 'top');
									myAlert.add({ className: "error", message: "Product structure not synched!" });
								});
								return;
		}  
		...
		var index=0;
	   	...
	   	if(typeof matchresult != 'undefined' && matchresult != null) {
	   		...
	   		if(matchresult == "left") {	
 				subscribe(channel3dPlayFrame1.viewer, false); 
	   			for(var k=0;k<idPath.length;k++){
	   			selectedRowIdsFrame1.push(idPath[k]);
		        }
		        selectedRowIdsFrame1 = selectedRowIdsFrame1.concat(getAllNodes3DPlay(rowNode, idPath, leftQtyIndex,channel3dPlayFrame.viewer,channel3dPlayFrame1.viewer,mode));
		        index= leftQtyIndex;
		        
	   		} else if(matchresult == "right") { 
	   			subscribe(channel3dPlayFrame.viewer, false); 
		        for(var k=0;k<idPath.length;k++){
		        selectedRowIdsFrame2.push(idPath[k]);    
		        }
		        selectedRowIdsFrame2 = selectedRowIdsFrame2.concat(getAllNodes3DPlay(rowNode, idPath, rightQtyIndex,channel3dPlayFrame1.viewer,channel3dPlayFrame.viewer,mode));
		        index= rightQtyIndex;
	   		} else if(matchresult == "common") {
	   			//for(var k=0;k<idPath.length;k++){
				//selectedRowIdsFrame1.push(idPath[k]);
				selectedRowIdsFrame1.push(idPath[0]);
				//}
		        selectedRowIdsFrame1 = selectedRowIdsFrame1.concat(getAllNodes3DPlay(rowNode, idPath, leftQtyIndex,channel3dPlayFrame.viewer,channel3dPlayFrame1.viewer,mode));	        
		        index= leftQtyIndex;
		       // idPath = getIDPathFromXMLNodeRight3DPlay(rowNode);
		        
		        //for(var k=0;k<idPath.length;k++){ 
		        //selectedRowIdsFrame2.push(idPath[k]);
				selectedRowIdsFrame2.push(idPath[1]);
		        //}
		        selectedRowIdsFrame2 = selectedRowIdsFrame2.concat(getAllNodes3DPlay(rowNode, idPath, rightQtyIndex,channel3dPlayFrame1.viewer,channel3dPlayFrame.viewer,mode));
		        index= rightQtyIndex;
	   		}
	   		...
	   		subscribe(channel3dPlayFrame.viewer, false);
	   		toggleSelect3DPlay(channel3dPlayFrame.viewer, mode, idPath);
	   		subscribe(channel3dPlayFrame.viewer, true);
	   		subscribe(channel3dPlayFrame1.viewer, true);
	   	}
   	}
 
   	quan = emxUICore.selectSingleNode(rowNode, "c[" + index + "]").attributes["a"].value;
   	if(parseInt(quan)<=1){
	//Multi select
	for(var counter=0; counter < selectedRowIdsFrame1.length; counter++) {
		...
		subscribe(channel3dPlayFrame.viewer, false);
		try{
		toggleSelect3DPlay(channel3dPlayFrame.viewer, mode, selectedRowIdsFrame1);
	...
	for(var counter=0; counter < selectedRowIdsFrame2.length; counter++) {
	    subscribe(channel3dPlayFrame1.viewer, false);
		toggleSelect3DPlay(channel3dPlayFrame1.viewer, mode, selectedRowIdsFrame2);
		subscribe(channel3dPlayFrame1.viewer, true);
		subscribe(channel3dPlayFrame.viewer, true);
	}
	}
	...
	if (selectedRows && selectedRows.length > 0) {
  		for(var i=0; i<selectedRows.length; i++){  		
		    ...
		    var partRev   = getValueForCell(selectedRows[i].getAttribute("id"),"Revision","display");
		  	...
		  	if(policy == "<%= POLICY_CONFIGURED_PART %>") {
		  	<!-- XSSOk -->
...
function validateUOM() {
    ...
    var RDValue = emxEditableTable.getCellValueByRowId(this.currentRow.getAttribute("id"),"Reference Designator").value.current.actual;
	    ...
	    if(sQtyValue % 1 != 0 && sUOMValue == "<%=rangeEAeach%>"){
     	...
     	<!-- XSSOK -->
     	else if("<%=isInstaceMode%>" == "true" || parentpolicy =="<%=EngineeringConstants.POLICY_CONFIGURED_PART%>"){
	     	<!-- XSSOK -->
	     	if(status != "add" && status != "new" && sQtyValue > 1 && sUOMValue == "<%=rangeEAeach%>"){
	     		<!-- XSSOk -->
	     		alert("<%=sUOMCanNotBeModified %>");
	     		return false;
	     	} 
	     }
	     	if(parentpolicy !="<%=EngineeringConstants.POLICY_CONFIGURED_PART%>"){
		     	if(!isFNUniqueOnInstanceWithAlertMsgOnFailure(curFN,alevel,currentobjId,parentObjectID,sUOMValue,VPMVisibleValue,RDValue,curRowId)){
					return false;
		     	}
	     	}
...
function onChangeReleaseProcess() {
    ...
    function clearSelectedWorkUnderChange() {
    	document.getElementById("WorkUnderChange_displayValue").value = "";
    	document.getElementById("WorkUnderChange_actualValue").value = "";
    	setRequestSetting("workUnderChangeId", "");
    }
    
    function closeAuthoringChooser() {
    	var newIcon = "../common/images/iconActionworkunder.png";
    	var newImage = "<img data-type=\"command\" src=\"../common/images/iconActionworkunder.png\">";
    	<!-- XSSOk -->
    	var newWorkUnderChangeTitle = "<%= selectWorkUnderChange %>";
    	
    	if ( document.getElementById("WorkUnderChange_displayValue").value != null && document.getElementById("WorkUnderChange_displayValue").value != "" ) {
    		newIcon = "../common/images/iconActionworkunderCheck.png";
    		newImage = "<img data-type=\"command\" src=\"../common/images/iconActionworkunderCheck.png\">";
    		<!-- XSSOK -->
    		newWorkUnderChangeTitle = "<%= workUnderChange %> : " + document.getElementById("WorkUnderChange_displayValue").value;
    	}
    	  
    	var objAuthoringChange = findToolbarItem( toolbars[0], "displayAuthoringChangeOnEdit()" );
    	objAuthoringChange.element.title = newWorkUnderChangeTitle;
    	objAuthoringChange.text = newWorkUnderChangeTitle;
    	objAuthoringChange.element.innerHTML = newImage;
    	objAuthoringChange.icon = newIcon;
    	
    	document.getElementById("AuthoringChangeChooser").style.display = "none";
    }

function displayAuthoringChangeOnEdit() {
	if (document.getElementById("AuthoringChangeChooser") == null || document.getElementById("AuthoringChangeChooser") == "undefined") {
			var authoringChangeDisplayText = document.createElement("input");
			authoringChangeDisplayText.setAttribute("type", "text");
			authoringChangeDisplayText.setAttribute("name", "WorkUnderChange_displayValue");
			authoringChangeDisplayText.setAttribute("id", "WorkUnderChange_displayValue");
			authoringChangeDisplayText.setAttribute("value", "");
			authoringChangeDisplayText.setAttribute("size", "25");
			authoringChangeDisplayText.setAttribute("style", "padding: 6px;");
			<!-- XSSOK -->
			authoringChangeDisplayText.setAttribute("placeholder", "<%= selectWorkUnderChange %>");
			authoringChangeDisplayText.setAttribute("readonly", "true");
			
			var authoringChangeActualHid = document.createElement("input");
			authoringChangeActualHid.setAttribute("type", "hidden");
			authoringChangeActualHid.setAttribute("name", "WorkUnderChange_actualValue");
			authoringChangeActualHid.setAttribute("id", "WorkUnderChange_actualValue");
			authoringChangeActualHid.setAttribute("value", "");
			
			var authoringChangeChooserBtn = document.createElement("input");
			authoringChangeChooserBtn.setAttribute("type", "button");
			authoringChangeChooserBtn.setAttribute("value", "...");
			authoringChangeChooserBtn.setAttribute("style", "padding: 6px;");
			authoringChangeChooserBtn.setAttribute("onclick", "javascript:showModalDialog(\"../common/emxFullSearch.jsp?field=TYPES=type_ChangeAction:CURRENT=policy_ChangeAction.state_InWork&txtPolicy=policy_ChangeAction&table=PUEUEContextChangeSearchTable&showInitialResults=false&includeOIDprogram=enoUnifiedBOM:getProposedChangeActionIds&objectId=" + objectId + "&selection=single&submitAction=refreshCaller&submitURL=../engineeringcentral/SearchUtil.jsp&fieldNameActual=WorkUnderChange_actualValue&fieldNameDisplay=WorkUnderChange_displayValue&mode=workUnderChange&sortColumnName=Name&portalMode=false&expandProgram=false&isIndentedView=false&showSavedQuery=True&searchCollectionEnabled=True\");");
			
			var authoringChangeClearImg = document.createElement("img");
			authoringChangeClearImg.setAttribute("src", "images/iconActionRefresh.gif");
			authoringChangeClearImg.setAttribute("onclick", "javascript:clearSelectedWorkUnderChange()");
						
			var authoringChangeChooserCloseBtn = document.createElement("input");
			authoringChangeChooserCloseBtn.setAttribute("type", "button");
			authoringChangeChooserCloseBtn.setAttribute("value", "X");
			authoringChangeChooserCloseBtn.setAttribute("style", "padding: 6px;");
			authoringChangeChooserCloseBtn.setAttribute("onclick", "javascript:closeAuthoringChooser();");
			
			var authoringChooserTable = document.createElement("table");
			var authoringChooserTableRow = document.createElement("tr");
			var authoringChooserTableColumn = document.createElement("td");			
			
			authoringChooserTableColumn.appendChild(authoringChangeDisplayText);
			authoringChooserTableColumn.appendChild(authoringChangeActualHid);
			authoringChooserTableColumn.appendChild(authoringChangeClearImg);
			authoringChooserTableColumn.appendChild(authoringChangeChooserBtn);
			authoringChooserTableColumn.appendChild(authoringChangeChooserCloseBtn);
			
			authoringChooserTableRow.appendChild(authoringChooserTableColumn);
			authoringChooserTable.appendChild(authoringChooserTableRow);
			
			var objAuthoringChange = findToolbarItem( objMainToolbar, "displayAuthoringChangeOnEdit()" );
			var iLeft = objAuthoringChange.element.offsetLeft + 10;
			var iTop = objAuthoringChange.element.offsetTop + 36;
			
			var divWidth = "290px";
			if (isChrome) { divWidth = "310px"; } 
			
			var authoringChangeChooserDiv = document.createElement("div");
			authoringChangeChooserDiv.setAttribute("id", "AuthoringChangeChooser");		
			authoringChangeChooserDiv.setAttribute("align", "center");
			authoringChangeChooserDiv.setAttribute("class", "formLayer");
			authoringChangeChooserDiv.setAttribute("style", "position: absolute; zIndex: 100; visibility: visible; margin: auto");
			authoringChangeChooserDiv.style.width = divWidth;
			authoringChangeChooserDiv.style.maxWidth = divWidth;
			authoringChangeChooserDiv.style.left = iLeft + "px";
			authoringChangeChooserDiv.style.top = iTop + "px";
			
			authoringChangeChooserDiv.appendChild(authoringChooserTable);
			
			document.getElementById("pageHeadDiv").appendChild(authoringChangeChooserDiv);
		}
		
		document.getElementById("AuthoringChangeChooser").style.display = "block";
}
...
function getDataUsingAjax(urlParam){
	var url = "../engineeringcentral/enoENCGetInstantData.jsp?"+urlParam;
	var result = emxUICore.getData(url);
	var re = new RegExp("[\r\n]", 'g');
...
function moveToEditMode(functionality){
  ...
  var selPartPolicy = getActualValueForOtherColumn("Policy",checkedRows[0]);
    if(ischild){
	...
	var urlParam = "functionality=checkifChangeEnabled&objectId="+objId;
	var changeControlledValue = getDataUsingAjax(urlParam);
	currentState = currentState.trim();
     	 if(!moveToEdit){
     	 if(tablemode=="view"){
     	 		if((changeControlledValue=="true" || changeControlledValue=="True") && (sbReference.getRequestSetting("ChangeAuthoringContextOID") == undefined || sbReference.getRequestSetting("ChangeAuthoringContextOID") == 'undefined' || sbReference.getRequestSetting("ChangeAuthoringContextOID")=="" || sbReference.getRequestSetting("ChangeAuthoringContextOID")== null)){
    	...
    	for (var i = 0; i < checkedRows.length; i++) {
	       ...
	       var childId = checkedRows[i].getAttribute("o");
      		...
      		var selParentPartPolicy = getActualValueForOtherColumn("Policy",checkedRows[i].parentNode);
			var urlParam = "functionality=checkifChangeEnabled&objectId="+parentId;
			var selParentChangeControlled = getDataUsingAjax(urlParam);
	       ...
	       if(!moveToEdit){
	       if(tablemode=="view"){
	       		if((selParentChangeControlled=="true" || selParentChangeControlled=="True") && (sbReference.getRequestSetting("ChangeAuthoringContextOID") == undefined || sbReference.getRequestSetting("ChangeAuthoringContextOID") == 'undefined' || sbReference.getRequestSetting("ChangeAuthoringContextOID")=="" || sbReference.getRequestSetting("ChangeAuthoringContextOID")== null)){
	       			alert("<emxUtil:i18nScript localize="i18nId">emxEngineeringCentral.Common.WorkUnderChangeRequired</emxUtil:i18nScript>");
	       			return;
	       		}
	       	}
	...
	if(displayView && displayView != null && displayView != undefined && displayView != "detail" && moveToEdit)
      {
      	if(isConfigContext && isConfigContext == true){
      	<!-- XSSOK -->
      		alert("<%=strEditInConfigContext%>");
      	}
      	else{
      	<!-- XSSOk -->
...
function createPart()
{
	...
	getTopWindow().showSlideInDialog("../engineeringcentral/PartCreatePreProcess.jsp?CreateMode=EBOM&multiPartCreation=true&selection=single&suiteKey=EngineeringCentral&objectId="+objId+"&emxTableRowId="+emxTableRowId,true,"","",550);
...
function getVPMVisibleValue() {
	var attrVPMVisibleValue = "";
	var attrVPMVisibleCellData = emxEditableTable.getCellValueByRowId(this.currentRow.getAttribute("id"),"VPMVisible");
	
	if(attrVPMVisibleCellData) 
		attrVPMVisibleValue = attrVPMVisibleCellData.value.current.display;
	
	return attrVPMVisibleValue;
}
