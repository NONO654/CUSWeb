...
<%
	...
	try{
		...
		else if("CAAffectedItemsAddExisting".equalsIgnoreCase(functionality))
		{
			...
			searchURL	+= "&table=AEFGeneralSearchResults&selection=multiple&showSavedQuery=True&searchCollectionEnabled=True&field=TYPES="+searchTypes+":PROJECT="+searchProject+"&submitURL=../enterprisechangemgtapp/ECMFullSearchPostProcess.jsp?submitAction=refreshCaller";
		}
		...
		{	
			...
			if(UIUtil.isNullOrEmpty(crId)){
				...
				searchURL	  += "&table=AEFGeneralSearchResults&hideHeader=true&selection=single&showSavedQuery=True&excludeOID="+objectId+"&HelpMarker=emxhelpfullsearch&searchCollectionEnabled=True&field=TYPES="+searchTypes+":PROJECT="+searchProject+"&submitURL=../enterprisechangemgtapp/ECMDisconnectProcess.jsp?submitAction=refreshCaller";
			} else{
				searchTypes    = "type_ChangeOrder:CURRENT=policy_FormalChange.state_Propose,policy_FormalChange.state_Prepare,policy_FasttrackChange.state_Prepare";
				// This selectable is not correct
				//:Organization=" + changeOrder.getInfo(context, orgNameSelect);
				searchURL	  += "&table=AEFGeneralSearchResults&includeOIDprogram=enoECMChangeRequest:includeCOOIDs&excludeOID="+objectId+"&hideHeader=true&selection=single&showSavedQuery=True&HelpMarker=emxhelpfullsearch&searchCollectionEnabled=True&field=TYPES="+searchTypes+":PROJECT="+searchProject+"&submitURL=../enterprisechangemgtapp/ECMDisconnectProcess.jsp?submitAction=refreshCaller";
			}
		}
		...
		{				
			...
			session.setAttribute("sourceAffectedItemRowIds",emxTableRowIds);
			searchURL  = "../common/emxCreate.jsp?form=type_CreateChangeOrderSlidein&type=type_ChangeOrder&CreateMode=CreateCO&typeChooser=true&header=EnterpriseChangeMgt.Command.CreateChange&nameField=autoname&createJPO=enoECMChangeOrder:createChange&submitAction=refreshCaller&preProcessJavaScript=preProcessInCreateCO&objectId="+objectId+"&functionality="+functionality+"&suiteKey="+suiteKey+"&postProcessURL=../enterprisechangemgtapp/ECMDisconnectProcess.jsp&HelpMarker=emxhelpchangeordercreate&objectId="+objectId+"&mode=create&SuiteDirectory=enterprisechangemgt";
		...
		{	
			...
			searchURL	  += "&table=AEFGeneralSearchResults&hideHeader=true&excludeOID="+ FrameworkUtil.join(caIdsList, ",") +"&selection=single&showSavedQuery=True&HelpMarker=emxhelpfullsearch&searchCollectionEnabled=True&field=TYPES="+searchTypes+":PROJECT="+searchProject+"&submitURL=../enterprisechangemgtapp/ECMFullSearchPostProcess.jsp?submitAction=refreshCaller";
		}
		else if ("AddToExistingChange".equalsIgnoreCase(functionality) || "ECMAddToExistingCO".equalsIgnoreCase(functionality)) 
		{							
			session.setAttribute("sourceAffectedItemRowIds",emxTableRowIds);
			searchTypes  = "type_ChangeOrder:CURRENT=policy_FormalChange.state_Propose,policy_FormalChange.state_Prepare,policy_FasttrackChange.state_Prepare";
			searchURL   += "field=TYPES="+searchTypes+":PROJECT="+searchProject+"&table=AEFGeneralSearchResults&selection=single&showSavedQuery=True&searchCollectionEnabled=True&functionality="+functionality+"&objectId="+objectId+"&formInclusionList=Description&field=TYPES="+searchTypes+"&submitURL=../enterprisechangemgtapp/ECMFullSearchPostProcess.jsp?submitAction=doNothing";
		...
		{							
			...
			session.setAttribute("sourceAffectedItemRowIds",AffeObjList);
			searchTypes  = "type_ChangeRequest:CURRENT=policy_ChangeRequest.state_Create,policy_ChangeRequest.state_Evaluate";
			searchURL   += "field=TYPES="+searchTypes+"&table=AEFGeneralSearchResults&selection=single&showSavedQuery=True&searchCollectionEnabled=True&functionality="+functionality+"&objectId="+objectId+"&formInclusionList=Description&submitURL=../enterprisechangemgtapp/ECMFullSearchPostProcess.jsp?submitAction=doNothing";
		...
		else if("AddCAReferential".equalsIgnoreCase(functionality)) 
		{
			...
			searchURL   += "&table=AEFGeneralSearchResults&selection=multiple&excludeOIDprogram=enoECMChangeAction:excludeReferentialOIDs&field=TYPES="+searchTypes+"&submitURL=../enterprisechangemgtapp/ECMFullSearchPostProcess.jsp?submitAction=refreshCaller&relName="+targetRelName+"";
		}
