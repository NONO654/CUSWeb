...
try {
	...
	String kind = emxGetParameter(request,"kind");
	boolean isTemplate = "T".equals(kind);
	boolean isOutputFormat = "OF".equals(kind);
	...
	if (isTemplate) services.deleteTemplates(context, memberIds);
	else if (isOutputFormat) services.deleteOutputFormats(context, memberIds);
} catch (ObjectUsedInReportException e1) {
